...
(function() {
	...
	e.prototype.initialize = function(f) {
		...
		this._domElement.className='Z-MAPjh';
		this._updateCss();
		this._updateText();
...
//		})
	...
	e.prototype._updateText = function() {
		if (this._domElement) {
			this._domElement.innerHTML ='<i>'+this._text+'</i>' 
		}
	};
	...
	e.prototype._buildCssText = function(g) {
		...
		var oHurl=h.slice(h.lastIndexOf("/"))
		if(oHurl=='/xiaor.gif'||oHurl=='/f3on.png'){//绿
			$('.Z-MAPjh i').css({'border-color':'#23fe32','background':'rgba(35,254,50,0.3)'});
		}else if(oHurl=='/f1on.png'||oHurl=='/f2on.png'||oHurl=='/h1on.png'||oHurl=='/e1on.png'||oHurl=='/c8on.png'||oHurl=='/car4on.png'){//黄
			$('.Z-MAPjh i').css({'border-color':'#ff9c00','background':'rgba(255,156,0,0.3)'});
		}else if(oHurl=='/f4on.png'||oHurl=='/c5on.png'||oHurl=='/c6on.png'||oHurl=='/h2on.png'||oHurl=='/h3on.png'){//红
			$('.Z-MAPjh i').css({'border-color':'#ff1414','background':'rgba(255,20,20,0.3)'});
		}else if(oHurl=='/h5on.png'){//消防水带
			$('.Z-MAPjh i').css({'border-color':'#f9ce9a','background':'rgba(249,206,154,0.3)'});
		}
		
		if (d.browser.ie < 7) {
			m.push('filter:progid:DXImageTransform.Microsoft.AlphaImageLoader(sizingMethod=scale,src="' + h + '");')
		} else {
			m.push("background:url(" + h + ") no-repeat;");
			var i = "0 0";
			(j instanceof BMap.Size) && (i = j.width + "px " + j.height + "px");
			m.push("background-position:center bottom;")
		}
		...
		m.push("cursor:pointer; color:" + l + "; position:absolute; font-size:" + f + "px; font-family:Arial,sans-serif; font-weight:bold");
 		...
 		//$('.Z-MAPjh').parent().css({'z-index':'401'})

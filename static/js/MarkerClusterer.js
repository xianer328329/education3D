...
(function(){
    ...
    MarkerClusterer.prototype._removeMarkersFromMap = function(){
        for(var i = 0, marker; marker = this._markers[i]; i++){
            marker.isInCluster = false;
            tmplabel = marker.getLabel(); // YPY
            this._map.removeOverlay(marker);
            marker.setLabel(tmplabel); // YPY
        }
    };
...
//        this.updateClusterMarker();
    ...
    // YPY
    Cluster.prototype._render = function() {
        var len = this._markers.length;
        if (len < this._minClusterSize) {
        	for (var i = 0; i < len; i++) {
        		try{
        			if(this._markers[i].content){
	            		var label=new BMap.Label('<div class="map_buildName">'+this._markers[i].content+'</div>',{});
	            		label.setStyle({

...
WebGLPlayer.prototype.initGL = function (options) {
    ...
    this.gl_options.v = gl.v;
    this.gl_options.u = gl.u;
    this.gl_options.y = gl.y;
    this.gl_options.texCoordBuffer = texCoordBuffer;
    this.gl_options.verticesBuffer = verticesBuffer;
    this.gl_options.fragmentShader = fragmentShader;
    this.gl_options.vertexShader = vertexShader;
    this.gl_options.program = program;
}
...
WebGLPlayer.prototype.renderFrame = function (videoFrame, width, height, uOffset, vOffset) {
	...
	if(gl.canvas.parentNode){//2021-08-09新加
		gl.canvas.width=gl.canvas.parentNode.offsetWidth;
...
WebGLPlayer.prototype.destroy = function () {
    if (!this.gl || !this.gl_options) {
        return;
    }

    var gl = this.gl;
    var opts = this.gl_options;

    //gl.deleteFramebuffer();
    //gl.deleteRenderbuffer();

    if (opts.v)
        gl.deleteTexture(opts.v.texture);
    if (opts.u)
        gl.deleteTexture(opts.u.texture);
    if (opts.y)
        gl.deleteTexture(opts.y.texture);

    if (opts.texCoordBuffer)
        gl.deleteBuffer(opts.texCoordBuffer);
    if (opts.verticesBuffer)
        gl.deleteBuffer(opts.verticesBuffer);

    if (opts.vertexShader)
        gl.deleteShader(opts.vertexShader);
    if (opts.fragmentShader)
        gl.deleteShader(opts.fragmentShader);

    if (opts.program)
        gl.deleteProgram(opts.program);

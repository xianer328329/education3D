<template>
	<el-form label-width="100px" :model="tableData" class="h-upload-form" ref="form">
		...
		<el-form-item label="人员名称" class="is-required" prop="controlName">
	...
	export default {
		...
		mounted() {
			...
			if(videoUrl.length>1){
				...
				this.$message.error('未配置人脸联防布控的请求地址')
			}
		},
		methods: {
			...
			async addControl() {
				...
				var facePicUrl=await this.uploadFile();				
				await this.localDistribution(facePicUrl);
...
//				vm.tableData.scopeUnit += ',' + UNITID;
				...
				params.append('facePicUrl', facePicUrl);
				...
				// params.append('function', 'addFace');
				this.$axios.post('monitor/initiateControl/' + UNITID, params,(data,mess)=>{					
					  this.loading=false;
			...
			// 本地下发
			async localDistribution(facePicUrl){
				let params={
					facePicUrl:facePicUrl,
					controlName:this.tableData.controlName,
					'function':'addFace'
				}
				return this.$axios.post(this.req,params,(data,mess)=>{
					this.loading=false;
					this.$message.error(mess);
				}).then(res=>{
					return res;
...
<style lang="scss">
	...
	.h-upload-form .avatar-uploader-icon {
		color: #8c939d;
		width: 9rem;
		height: 9rem;
	...
	.h-upload-form .avatar-uploader .el-upload {
	...
	.h-upload-form .avatar-uploader .el-upload:hover {
		border-color: #409EFF;
	}
</style>
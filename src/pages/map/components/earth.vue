   <template>
	<div class="h-container">
	 	<div class="earth" :class="{'h-opacity':showEarth}" ref="earth"></div>
	 	<div class="h-mask"></div>
	 	<div class="starts-c" :class="className">
	 		<div class="stars" ></div>
	 	</div>
	</div>
</template>
...
<script>
	/* eslin -disable */
	import earth from '../data-gl/earthLine.js';
	var timer,timer1,earthChart;
	export default {
		data(){
			return {
				deg:30,
		...
		methods:{
			init(){
				...
				this.$nextTick(function(){
					vm.className='h-scale-star';
					this.initEarth();
					sessionStorage.setItem('mapIndex',1);
					timer=setTimeout(function(){
						window.ROUTER.push('/map');
					},Math.floor(210/vm.deg)*1000)
				});
			},
			initEarth(){
				var vm=this;
				try{
					earthChart=echarts.init(vm.$refs.earth);
					earthChart.setOption(vm.getOption());
					earthChart.on('finished', function () {
			...
			getOption(){
				...
				return  {
				    ...
				    globe: {
				        baseTexture:require('../data-gl/asset/earth.jpg'),
				        heightTexture:require('../data-gl/asset/bathymetry_bw_composite_4k.jpg'),
						globeRadius:50,
...
//				        environment: require('../data-gl/asset/starfield.jpg'),
						environment:'rgba(0,0,0,0.2)',
						viewControl:{
							autoRotateSpeed :vm.deg,
							alpha:35,
						},
				        light: {
				            ambient: {
				        ...
				        layers: [{
				            type: 'blend',
				            blendTo: 'emission',
				            texture: require('../data-gl/asset/night.jpg'),
				        }, {
				            type: 'overlay',
				            texture:require('../data-gl/asset/clouds.png'),
				            shading: 'lambert',
				            distance: 5
				        }]
				    },
...
.earth{
	width: 1920px;
	height:1080px;
	/*background:url(../data-gl/asset/starfield.jpg) no-repeat center;*/
	opacity: 1;
	/*background-size:100% 100%;*/
